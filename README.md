...
[![Version](https://img.shields.io/badge/version-v1.0.0-blue.svg)](https://github.com/Lexo0522/one-key-dialer)
...
`PPoEDialer` 是 **薄 Swing shell**（接线 / Host / 生命周期）；业务在 `service/*`，UI 在 `ui/*`，持久化在 `storage/*`。
...
版本号：**v1.0.0**（`APP_VERSION` 为 `v1.0.0`；`pom.xml` / `jpackage --app-version` / HTTP User-Agent 为 `1.0.0`，须保持同步）
